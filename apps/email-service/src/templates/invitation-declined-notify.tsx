...
import { InvitationDeclinedNotifyPayload } from '../types.js';

export function InvitationDeclinedNotifyEmail({ firstName, declinerName, orgName }: InvitationDeclinedNotifyPayload) {
  return (
    <Html lang="es">
      <Head />
      <Preview>{declinerName} rechazó tu invitación a {orgName}</Preview>
      <Body style={body}>
        <Container style={container}>
          <Section style={iconSection}><div style={iconCircle}>ℹ️</div></Section>
          <Heading style={h1}>Invitación rechazada</Heading>
          <Text style={text}>
            Hola {firstName}, <strong>{declinerName}</strong> rechazó tu invitación para unirse a{' '}
            <strong>{orgName}</strong>.
          </Text>
          <Text style={text}>
            Puedes enviarle una nueva invitación cuando lo consideres oportuno.
          </Text>
          <Hr style={hr} />
