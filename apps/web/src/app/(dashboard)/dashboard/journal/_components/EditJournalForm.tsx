...
import { updateJournalEntryAction } from "@/lib/actions";
import { ApiError } from "@/lib/api-client";
import type { Account, JournalEntry } from "@accounting/types";
...
function entryToLines(entry: JournalEntry): LineRow[] {
...
export function EditJournalForm({ entry, accounts }: { entry: JournalEntry; accounts: Account[] }) {
  ...
  const [lines, setLines]           = useState<LineRow[]>(entryToLines(entry));
  ...
  function addLine() {
    setLines((prev) => [...prev, { id: nextId++, accountId: "", debit: "", credit: "", note: "" }]);
  }
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-800">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <Field label="Fecha" error={errors.date}>
            <input type="date" value={date} onChange={(e) => { setDate(e.target.value); setErrors({}); }} className={inp(!!errors.date)} />
          </Field>
          <Field label="Descripción" error={errors.description} className="sm:col-span-2">
            <input type="text" value={description} onChange={(e) => { setDescription(e.target.value); setErrors({}); }} className={inp(!!errors.description)} />
          </Field>
          <Field label={<>Referencia <span className="font-normal text-slate-400">(opcional)</span></>} error={errors.reference}>
            <input type="text" value={reference} onChange={(e) => { setReference(e.target.value); setErrors({}); }} className={inp(!!errors.reference)} />
          </Field>
        </div>
      </div>
              ...
              {lines.map((line, i) => (
                <tr key={line.id}>
                  <td className="px-4 py-2">
                    <div>
                      <select value={line.accountId} onChange={(e) => updateLine(line.id, "accountId", e.target.value)} className={inp(!!errors[`line_${i}_account`])}>
                        <option value="">Selecciona cuenta</option>
                        {accounts.map((a) => (
                          <option key={a.id} value={a.id}>{a.code} — {a.name}</option>
                        ))}
                      </select>
                      {errors[`line_${i}_account`] && <p className="mt-0.5 text-xs text-red-600">{errors[`line_${i}_account`]}</p>}
                    </div>
                  </td>
                  <td className="px-4 py-2">
                  ...
                  <td className="px-4 py-2">
                    <div>
                      <input type="number" min="0" step="0.01" placeholder="0.00" value={line.credit}
                        ...
                        className={`${inp(!!errors[`line_${i}_amount`])} text-right`} />
                      {errors[`line_${i}_amount`] && <p className="mt-0.5 text-xs text-red-600">{errors[`line_${i}_amount`]}</p>}
                    </div>
                  </td>
                  <td className="px-4 py-2 hidden md:table-cell">
                    <input type="text" placeholder="Opcional" value={line.note}
                      onChange={(e) => updateLine(line.id, "note", e.target.value)} className={inp(false)} />
                  </td>
        ...
        <div className="border-t border-slate-200 px-4 py-3 dark:border-slate-700">
          <div className="flex items-center justify-end gap-8">
            <div className="text-right">
              ...
              <p className="font-mono text-sm font-semibold text-slate-900 dark:text-slate-100">{fmt(totalDebit)}</p>
            </div>
            <div className="text-right">
              <p className="text-xs text-slate-500">Total créditos</p>
              <p className="font-mono text-sm font-semibold text-slate-900 dark:text-slate-100">{fmt(totalCredit)}</p>
            </div>
