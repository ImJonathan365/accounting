...
import type { UserOrg } from "@accounting/types";
...
export function UserMenu({ displayName, initials, orgs, currentOrgId }: Props) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 w-56 origin-top-right rounded-xl border border-slate-200 bg-white shadow-lg ring-1 ring-black/5 dark:border-slate-700 dark:bg-slate-800">
          {/* User header */}
          <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-700">
            ...
            {currentOrg && (
              <p className="mt-0.5 truncate text-xs text-slate-400 dark:text-slate-500">
                {currentOrg.name}
                <span className="ml-1.5 rounded-full bg-indigo-50 px-1.5 py-0.5 text-[10px] font-medium text-indigo-600 dark:bg-indigo-950/50 dark:text-indigo-400">
                  {currentOrg.role}
                </span>
              </p>
            )}
          ...
          {otherOrgs.length > 0 && (
            <div className="border-b border-slate-100 py-1 dark:border-slate-700">
              <p className="px-4 pb-1 pt-2 text-[10px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">
              ...
              {otherOrgs.map((org) => (
                <button
                  ...
                  className="flex w-full items-center gap-2.5 px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 disabled:opacity-50 dark:text-slate-300 dark:hover:bg-slate-700/50 transition-colors"
                >
                  {switching === org.id ? (
                  ...
                  <span className="text-[10px] text-slate-400 dark:text-slate-500">{org.role}</span>
                </button>
              ))}
            </div>
          )}
...
function ChevronIcon({ open }: { open: boolean }) {
  return (
    <svg className={`h-3.5 w-3.5 transition-transform duration-150 ${open ? "rotate-180" : ""}`}
